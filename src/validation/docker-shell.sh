...
docker build -t data-validation --platform=linux/arm64/v8 -f Dockerfile .

# Run Container
docker run --rm --name data-validation -ti \
-v "$BASE_DIR":/app \
...
--network data-versioning-network data-validation
