...
docker build -t data-versioning --platform=linux/arm64/v8 -f Dockerfile .

# Run Container
docker run --rm --name data-versioning -ti \
-v "$BASE_DIR":/app \
...
--network data-versioning-network data-versioning